...
class P4PDevCleanupTests(unittest.TestCase):
    ...
    def test_public_site_build_generates_catalog_and_facts(self) -> None:
        ...
        providers_page_ar = (WORKSPACE_ROOT / "public/www/pizza4people/providers/ar.html").read_text(encoding="utf-8")
        ...
        backup_print_module_page_ar = (
            WORKSPACE_ROOT / "public/www/pizza4people/modules/p4p.order.print.backup/ar.html"
        ).read_text(encoding="utf-8")
        notify_sms_module_page_ar = (
            WORKSPACE_ROOT / "public/www/pizza4people/modules/p4p.notify.sms/ar.html"
        ).read_text(encoding="utf-8")
        ...
        provider_detail_page_ar = (
            WORKSPACE_ROOT / "public/www/pizza4people/providers/p4p.reference/ar.html"
        ).read_text(encoding="utf-8")
        ...
        self.assertIn("Kundeside", modules_page_da)
        ...
        self.assertIn("واجهة العميل", modules_page_ar)
        self.assertIn("الحالة الحالية", modules_page_ar)
        ...
        self.assertIn('<html lang="ar" dir="rtl">', providers_page_ar)
        self.assertIn("صفحات مزوّدي Pizza4People", providers_page_ar)
        ...
        self.assertIn("اقرأ التالي", module_page_ar)
        ...
        self.assertIn('<html lang="ar" dir="rtl">', provider_detail_page_ar)
        self.assertIn("ما الذي ينشره هذا المزوّد الآن.", provider_detail_page_ar)
        self.assertIn("قائمة طعام بسيطة على الويب", provider_detail_page_ar)
        self.assertIn("مسار الطابعة الاحتياطية", provider_detail_page_ar)
        self.assertIn("هذه الصفحة المحلية هي الواجهة المقروءة لـ P4P.", provider_detail_page_ar)
    ...
    def test_lab_gui_is_scenario_config_driven(self) -> None:
        ...
        nodes_page = (REPO_ROOT / "lab/nodes.html").read_text(encoding="utf-8")
        servers_page = (REPO_ROOT / "lab/servers.html").read_text(encoding="utf-8")
        control_page = (REPO_ROOT / "lab/control.html").read_text(encoding="utf-8")
        runs_page = (REPO_ROOT / "lab/runs.html").read_text(encoding="utf-8")
        topology_page = (REPO_ROOT / "lab/topology.html").read_text(encoding="utf-8")

        flow_ids = {scenario["id"] for scenario in projection["flow_scenarios"]}
        legacy_ids = {scenario["id"] for scenario in projection["legacy_scenarios"]}
        service_ids = set(projection["services"])
        registry_profile_ids = {profile["id"] for profile in projection["registry_profiles"]}
        profile_ids = {profile["id"] for profile in projection["node_profiles"]}
        template_ids = {profile["id"] for profile in projection["node_templates"]}
        instance_ids = {instance["id"] for instance in projection["node_instances"]}
        fixture_ids = {fixture["id"] for fixture in projection["customer_fixtures"]}

        self.assertIn("single-pizza-order", flow_ids)
        self.assertIn("fail-primary-keep-backup", flow_ids)
        self.assertIn("registry-client-demo-node", legacy_ids)
        self.assertIn("pilot-photo-map", service_ids)
        self.assertIn("primary-registry", service_ids)
        self.assertIn("secondary-registry", service_ids)
        self.assertIn("curated-backup-registry", service_ids)
        self.assertIn("main-open", registry_profile_ids)
        self.assertIn("dk-p4p-curated", registry_profile_ids)
        self.assertIn("pizza-shop", profile_ids)
        self.assertEqual(profile_ids, template_ids)
        self.assertIn("kebab-shop", profile_ids)
        self.assertIn("pizza-shop", instance_ids)
        self.assertIn("menu-only", instance_ids)
        self.assertIn("pickup-basic", fixture_ids)
        ...
        self.assertEqual(config.node_profiles["pizza-shop"].service_name, "pilot-pizza-shop")
        self.assertIn("main-open", config.node_profiles["pizza-shop"].registry_targets)
        self.assertEqual(
            config.node_profiles["pizza-shop"].customer_urls["public_shop"],
            "http://127.0.0.1:8595/p4p/menu/list",
        )
        self.assertEqual(config.customer_fixtures["pickup-basic"].target_profile, "pizza-shop")
        self.assertEqual(config.flow_scenarios[0].fixture_runs[0].fixture_id, "pickup-basic")
        self.assertIn("/api/scenarios", page)
        self.assertIn("/api/node-instances", page)
        self.assertIn("/api/node-instances/", page)
        self.assertIn("/control", page)
        self.assertIn("/runs", page)
        self.assertIn("/servers", page)
        self.assertIn("/topology", page)
        self.assertIn("lab/scenarios.json", page)
        self.assertIn("Infrastructure", page)
        self.assertIn("Node Templates", page)
        self.assertIn("Node Instances", page)
        self.assertIn("Customer Surfaces", page)
        self.assertIn("Customer Fixtures", page)
        self.assertIn("/api/customer-fixture-instances", page)
        self.assertIn("Create local fixture", page)
        self.assertIn("Step 1 · Name the fixture", page)
        self.assertIn("Step 4 · Build the order", page)
        self.assertIn("Fixture preview", page)
        self.assertIn("Local fixture preview", page)
        self.assertIn("Fixture lane truth", page)
        self.assertIn("Target owner", page)
        self.assertIn("Public/customer layer", page)
        self.assertIn("Registry path", page)
        self.assertIn('data-add-builder-row="fixture_items"', page)
        self.assertIn("Save local fixture", page)
        self.assertIn("Legacy Proof", page)
        self.assertIn("Edit instance-owned state", page)
        self.assertIn("Node id", page)
        self.assertIn("Order mode", page)
        self.assertIn("Categories", page)
        self.assertIn("Primary public surface key", page)
        self.assertIn("Menu seed JSON", page)
        self.assertIn("Node draft preview", page)
        self.assertIn("Node instance preview", page)
        self.assertIn("Runtime story", page)
        self.assertIn("Runtime lane", page)
        self.assertIn("What this changes", page)
        self.assertIn("Edit step 2 · Topology and public lane", page)
        self.assertIn("Edit step 3 · Runtime shape", page)
        self.assertIn("Save instance", page)
        self.assertIn("/api/node-profiles/", page)
        self.assertIn("/api/customer-fixtures/", page)
        self.assertIn("/api/flow-scenarios/", page)
        self.assertIn("/api/flow-scenario-instances/", page)
        self.assertIn('data-add-builder-row="services"', page)
        self.assertIn('data-add-builder-row="node_refs"', page)
        self.assertIn('data-add-builder-row="fixture_runs"', page)
        self.assertIn('data-add-builder-row="checks"', page)
        self.assertIn("Add fixture run", page)
        self.assertIn("Step 2 · Choose infrastructure", page)
        self.assertIn("Step 4 · Choose customer runs", page)
        self.assertIn("Step 5 · Define checks", page)
        self.assertIn("Refresh suggested checks", page)
        self.assertIn("Flow preview", page)
        self.assertIn("Local flow preview", page)
        self.assertIn("Flow lane truth", page)
        self.assertIn("Local owners", page)
        self.assertIn("Public/customer layers", page)
        self.assertIn("Select node target", page)
        self.assertIn("/api/flow-scenario-instances", page)
        self.assertIn("/api/node-instances/", page)
        self.assertIn("registry_profiles", page)
        self.assertIn("Create local flow", page)
        self.assertIn("Save local flow", page)
        self.assertIn("What the recent runs say", page)
        self.assertIn("Run story", page)
        self.assertIn("What this proved", page)
        self.assertIn("Owners touched", page)
        self.assertIn("Registry lanes", page)
        self.assertIn("P4P Lab Servers", servers_page)
        self.assertIn("/api/servers", servers_page)
        self.assertIn("What is alive right now", servers_page)
        self.assertIn("Port map", servers_page)
        self.assertIn("Main reading", servers_page)
        self.assertIn("/api/server-groups/", servers_page)
        self.assertIn("/api/server-actions/", servers_page)
        self.assertIn("Services in this room", servers_page)
        self.assertIn("Truth owners in this room", servers_page)
        self.assertIn("Dependencies first", servers_page)
        self.assertIn("Bring up in this order", servers_page)
        self.assertIn("Stop in this order", servers_page)
        self.assertIn("Recover rebinds these owners", servers_page)
        self.assertIn("Open this lane first", servers_page)
        self.assertIn("Port story", servers_page)
        self.assertIn("Open this first", servers_page)
        self.assertIn("Recover path", servers_page)
        self.assertIn("Truth owner", servers_page)
        self.assertIn("Adopt and bind", servers_page)
        self.assertIn("Depends on", servers_page)
        self.assertIn("Restart", servers_page)
        self.assertIn("Recover", servers_page)
        self.assertIn("Adopt", servers_page)
        self.assertIn("Rebind", servers_page)
        self.assertIn("adopted external", servers_page)
        self.assertIn("What lives here", servers_page)
        self.assertIn("Attached node instances", servers_page)
        self.assertIn("P4P Lab Nodes", nodes_page)
        self.assertIn("/api/nodes-room", nodes_page)
        self.assertIn("Seed templates in this room", nodes_page)
        self.assertIn("Filter these node lanes", nodes_page)
        self.assertIn("Matching node lanes", nodes_page)
        self.assertIn("Create local node instance", nodes_page)
        self.assertIn("Node instances in this room", nodes_page)
        self.assertIn("Customer/public surfaces in this room", nodes_page)
        self.assertIn("Family", nodes_page)
        self.assertIn("Order mode", nodes_page)
        self.assertIn("Registry lane", nodes_page)
        self.assertIn("Owner kind", nodes_page)
        self.assertIn("Public/customer surface", nodes_page)
        self.assertIn("Node draft preview", nodes_page)
        self.assertIn("Open this lane first", nodes_page)
        self.assertIn("Registry path", nodes_page)
        self.assertIn("Customer/public shape", nodes_page)
        self.assertIn("Node identity", nodes_page)
        self.assertIn("Truth owner", nodes_page)
        self.assertIn("P4P Lab Control Rooms", control_page)
        self.assertIn("/api/control-rooms", control_page)
        self.assertIn("What these rooms control", control_page)
        self.assertIn("Actionable rooms", control_page)
        self.assertIn("Open this room first", control_page)
        self.assertIn("What this room is for", control_page)
        self.assertIn("Start path", control_page)
        self.assertIn("Recovery path", control_page)
        self.assertIn("Truth owners in this room", control_page)
        self.assertIn("Services in this room", control_page)
        self.assertIn("Open the attached layers", control_page)
        self.assertIn("/api/server-groups/", control_page)
        self.assertIn("P4P Lab Runs", runs_page)
        self.assertIn("/api/runs-room", runs_page)
        self.assertIn("What the recent runs say", runs_page)
        self.assertIn("Latest proof", runs_page)
        self.assertIn("Filter these runs", runs_page)
        self.assertIn("Matching runs", runs_page)
        self.assertIn("Proof lanes in this room", runs_page)
        self.assertIn("First matching run", runs_page)
        self.assertIn("Owner lane", runs_page)
        self.assertIn("Public/customer layer", runs_page)
        self.assertIn("Registry lane", runs_page)
        self.assertIn("Show matching runs", runs_page)
        self.assertIn("Owners and layers touched", runs_page)
        self.assertIn("Run story", runs_page)
        self.assertIn("Run contract", runs_page)
        self.assertIn("What this proved", runs_page)
        self.assertIn("Public/customer layers", runs_page)
        self.assertIn("Registry lanes", runs_page)
        self.assertIn("Run data", runs_page)
        self.assertIn("P4P Lab Topology", topology_page)
        self.assertIn("/api/topology", topology_page)
        self.assertIn("/api/registry-profiles/", topology_page)
        self.assertIn("/api/registry-lane-instances", topology_page)
        self.assertIn("Lane runtime story", topology_page)
        self.assertIn("Attached node templates", topology_page)
        self.assertIn("Upstreams", topology_page)
        self.assertIn("Edit local lane metadata", topology_page)
        self.assertIn("Save local lane override", topology_page)
        self.assertIn("Create local registry lane", topology_page)
        self.assertIn("Save registry lane", topology_page)
        self.assertIn("Step 1 · Choose the lane seed", topology_page)
        self.assertIn("Registry lane preview", topology_page)
        self.assertIn("Edit step 2 · Set topology meaning", topology_page)
        self.assertIn("Lane metadata preview", topology_page)
        self.assertIn("announce here", topology_page)
        self.assertIn("visible here", topology_page)
