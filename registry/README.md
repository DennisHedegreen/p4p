...
- a registry-source snapshot must keep `latest_identity_event_id` aligned with the actual `identity_events` list; downstream import rejects inflated or stale pointers instead of letting mirror status misreport how much identity-log evidence the payload really carries
